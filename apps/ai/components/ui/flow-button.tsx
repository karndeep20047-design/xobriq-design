...
export const FlowButton = React.forwardRef<HTMLAnchorElement, FlowButtonProps>(
  ({ href = "/register", text, children, className = "", ...props }, ref) => {
    ...
    const content = (
      <>
        {/* Left arrow (arr-2) */}
        <ArrowRight className="absolute left-[-25%] z-[9] h-4 w-4 fill-none stroke-x-bg transition-[left,stroke] duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] group-hover:left-4 group-hover:stroke-x-fg" />

        {/* Text */}
        <span className="relative z-[1] -translate-x-3 text-x-bg transition-[transform,color] duration-300 ease-out group-hover:translate-x-3 group-hover:text-x-fg">
          {labelText}
        </span>

        {/* Expanding circle background */}
        <span className="absolute left-1/2 top-1/2 h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full bg-x-bg opacity-0 transition-[width,height,opacity,background-color] duration-400 ease-[cubic-bezier(0.19,1,0.22,1)] group-hover:h-[260px] group-hover:w-[260px] group-hover:opacity-100" />

        {/* Right arrow (arr-1) */}
        <ArrowRight className="absolute right-4 z-[9] h-4 w-4 fill-none stroke-x-bg transition-[right,stroke] duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] group-hover:right-[-25%] group-hover:stroke-x-fg" />
      </>
    );
