  ...
  ({ href, children, className = "", ...props }, ref) => {
    return (
      <Link
        ref={ref}
        href={href}
        className={`group relative inline-flex h-12 sm:h-13 min-h-[48px] sm:min-h-[52px] items-center justify-center overflow-hidden rounded-xl sm:rounded-full border border-x-line-strong dark:border-blue-500/80 font-semibold text-sm transition-[border-color,box-shadow,transform] duration-300 hover:border-x-accent/60 hover:shadow-[0_0_25px_rgba(63,169,255,0.35)] active:scale-98 ${className}`}
        {...props}
      >
        <div className="inline-flex h-full w-full translate-y-0 items-center justify-center px-6 sm:px-8 bg-x-raised dark:bg-[#0072c4] text-x-fg dark:text-white transition-transform duration-300 group-hover:-translate-y-[150%] font-semibold">
          {children}
        </div>
