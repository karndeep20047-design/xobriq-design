...
export function VerificationsListClient({
  ...
  }, [rows, query, status, doc, dateFrom, dateTo]);

  function handleExport() {
    downloadCsv(
      `xobriq-kyc-verifications-${new Date().toISOString().slice(0, 10)}.csv`,
      ["Reference", "Applicant", "Number", "Type", "Requested by", "Match", "Status", "Date"],
      filtered.map((v) => [
        v.ref,
      ...
      <Card className="border-border/60 shadow-[var(--shadow-card)]">
        <CardContent className="flex flex-col gap-3 p-4 md:flex-row md:items-center">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              variant={moreOpen || dateFrom || dateTo ? "default" : "outline"}
              className="gap-2"
        ...
        {moreOpen ? (
          <CardContent className="flex flex-wrap items-center gap-3 border-t border-border/60 p-4 pt-4">
          ...
          .map((s) => {
            ...
            return (
              <Button
                ...
                size="sm"
                className={cn("h-8 gap-1.5 rounded-full px-3 text-xs font-medium", !active && tone)}
              ...
              <TableBody>
                {filtered.map((v) => (
                  <TableRow key={v.ref} className="hover:bg-muted/40">
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-xs text-muted-foreground">{v.numberMasked}</div>
                        </div>
                      </div>
                    </TableCell>
                    ...
                    <TableCell>
                      <Badge variant="outline" className={cn("font-medium", statusStyles[v.status])}>
                        {v.status}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right">
                      <Button asChild size="sm" variant="ghost" className="gap-1">
                        <Link href={`${BASE}/verifications/${encodeURIComponent(v.ref)}`}>
                          View
