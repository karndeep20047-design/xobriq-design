// apps/ai/lib/email/send-org-invite.ts
import "server-only";
import { sendGraphMail } from "./graph";
import { SENDER_ADDRESS } from "./routing";
import { orgInviteEmail } from "./invite-template";
import { createAdminClient } from "@/lib/supabase/admin";

export type SendOrgInviteInput = {
...
// Mirrors send-staff-invite.ts's shape exactly, for a client organization
// invite instead of the internal console.
export async function sendOrgInviteEmail(
  ...
  try {
    ...
    console.error("[org-invite] send failed:", err instanceof Error ? err.message : String(err));
